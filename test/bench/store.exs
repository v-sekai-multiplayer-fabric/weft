...
#
# `Store.Sqlite` uses the default rollback journal, and it is the store every actor uses.
# `Store.Fdb` writes each key straight to FoundationDB, so the pair bounds the cost of a
# local commit against a network one.
