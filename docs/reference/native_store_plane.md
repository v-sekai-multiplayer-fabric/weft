...
`soak.sh` alternates two rounds. A load round runs the benchmark and then asks SQLite to
check itself. Every third round is adversarial: a writer is killed with SIGKILL while a
commit is in flight, and the kill delay walks across the window rather than sitting in
one place.

56 rounds in 70 seconds, with read and write rates steady and no drift:

```
44     55        load       1277372        1760           ok
45     56        kill+815   -              -              ok
46     57        load       1283936        1668           ok
...
rounds: 56, failures: 8, seconds: 71
```

**Eight rounds failed and the reason is not known.** Every printed check says `ok`, so
the failure is in the exit status of the crash program rather than in an integrity
check. It is not diagnosed, and it must be diagnosed before this VFS holds anything.

The soak also found three faults in itself, which is the point of running one:

- **No output at all.** `docker compose run` without `-T` wants a terminal, so the first
  soak ran blind for eighteen minutes and produced nothing.
- **The memory column measured the wrong process.** It read `/proc/self/status`, which is
  the shell, not the program under test. The column is gone rather than wrong.
- **CRLF line endings stopped the script.** Git converted the file on checkout, so the
  container read `set -eu` as `set -eu` and refused it. `.gitattributes` now forces LF
...
4. **Strip SQLite from the BEAM.** The store plane owns SQLite, so the BEAM must not.
   Delete `Weft.Actor.Store.Sqlite`, `Weft.Actor.Store.Replicated`, `.Replicator`, and
   `Weft.Actor.load_all`, and drop the `exqlite` dependency. The BEAM then reaches the
   store plane over iceoryx and holds no database of its own. This also removes the
   FoundationDB compaction race, because it removes the code that has it.
